...
pub fn show_contextmenu_command(window: Window, x: f64, y: f64) -> Result<(), String> {
    ...
    let cut_item = MenuItem::with_id(&window, "cut", "剪切", true, Option::<&str>::None)
        .map_err(|e| e.to_string())?;
    let copy_item = MenuItem::with_id(&window, "copy", "复制", true, Option::<&str>::None)
        .map_err(|e| e.to_string())?;
    let paste_item = MenuItem::with_id(&window, "paste", "粘贴", true, Option::<&str>::None)
        .map_err(|e| e.to_string())?;

    let rename_item = MenuItem::with_id(&window, "rename", "重命名", true, Option::<&str>::None)
        .map_err(|e| e.to_string())?;
    let delete_item = MenuItem::with_id(&window, "delete", "删除", true, Option::<&str>::None)
        .map_err(|e| e.to_string())?;
