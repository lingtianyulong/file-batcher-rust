...
async function handleOpenBatchDialog() {
  if (fileInfo.value.length === 0) {
    await message("请先打开文件或文件夹", {title: "提示", kind: "info"});
    return;
  }
  try {
    await invoke("open_batch_window_command");
  } catch (e) {
    await message(`打开批量操作窗口失败：${String(e)}`, {title: "错误", kind: "error"});
  }
}
...
async function handleOpenFolder() {
  ...
  try {
    ...
    await message(`打开文件夹失败：${String(error)}`, {title: "错误", kind: "error"});
  } finally {
...
async function handleRenameConfirm() {
  if (!currentRenameRow.value) {
    await message("未找到要重命名的文件", {title: "警告", kind: "warning"});
    return;
  }

  const newName = renameFileName.value.trim();
  if (!newName) {
    await message("请输入新的文件名", {title: "提示", kind: "info"});
    return;
  }

  if (newName === currentRenameRow.value.fileName) {
    await message("新文件名不能与原文件名相同", {title: "错误", kind: "error"});
    return;
  }
  ...
  try {
    ...
    await message("文件重命名成功", {title: "成功", kind: "info"});
  } catch (error) {
    await message(String(error), {title: "错误", kind: "error"});
  }
}
